...
- **Lock system** — `src/main/breakReminder.js` is the brain: a presence timer → beeping alarm → a fullscreen kiosk lock. The prompt offers only "I'm taking a break" (locks immediately) and "Approve me to keep playing" (pings Telegram watchers + a short "get up and check" lock). The kiosk lock window lives in `main.js` (`showLock`/`updateLock`/`hideLock`), renders `src/renderer/lock.html`, and swallows escape shortcuts via `globalShortcut` + `before-input-event`. Note: the Windows Secure Attention Sequence (Ctrl+Alt+Del) can't be blocked from user space — `src/main/taskmgrBlock.js` instead blocks Task Manager itself (the actual escape hatch) via the per-user `HKCU\...\Policies\System\DisableTaskMgr` registry value, which needs no admin/UAC elevation. It's toggled in `showLock`/`hideLock` and defensively cleared on every app start and quit so a crash mid-lock can never leave it stuck on.
